...
def table_factory_foolfuuka_threads(Base, board_name):# Threads table
    """Asagi / Foolfuuka style DB table
    ...
    logging.debug(u'table_factory_foolfuuka_threads() args={0!r}'.format(locals()))# Record arguments.
    assert(type(board_name) in [unicode])
    table_name = u'{0}_threads'.format(board_name)
    logging.debug(u'Naming the Foolfuuka thread table {0!r}'.format(table_name))
    class FoolfuukaThreads(Base):
    ...
    return FoolfuukaThreads



def table_factory_foolfuuka_posts(Base, board_name):# Posts table
    """Asagi / Foolfuuka style DB table
    Make the 'x_threads' table for an 4chan board.
    see https://stackoverflow.com/questions/19163911/dynamically-setting-tablename-for-sharding-in-sqlalchemy
    TODO: Sane database design"""
    logging.debug(u'table_factory_foolfuuka_posts() args={0!r}'.format(locals()))# Record arguments.
    assert(type(board_name) in [unicode])
    table_name = u'{0}'.format(board_name)
    logging.debug(u'Naming the Foolfuuka post table {0!r}'.format(table_name))
    class FoolfuukaPosts(Base):
        __tablename__ = table_name
        # from asagi boards.sql
        ...
        row_created = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, default=datetime.datetime.utcnow)
        row_updated = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    return FoolfuukaPosts



def table_factory_foolfuuka_images(Base, board_name):# images table
    """Asagi / Foolfuuka style DB table
    Make the 'x_files' table for an 4chan board.
    see https://stackoverflow.com/questions/19163911/dynamically-setting-tablename-for-sharding-in-sqlalchemy
    TODO: Sane database design"""
    logging.debug(u'table_factory_foolfuuka_images() args={0!r}'.format(locals()))# Record arguments.
    assert(type(board_name) in [unicode])
    table_name = u'{0}_images'.format(board_name)
    logging.debug(u'Naming the Foolfuuka images table {0!r}'.format(table_name))
    class FoolfuukaImage(Base):
        __tablename__ = table_name
        ...
        row_created = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, default=datetime.datetime.utcnow)
        row_updated = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    return FoolfuukaImage



# ===== Fuuka-style =====

def table_factory_fuuka(Base, board_name):
    table_name = u'{0}'.format(board_name)
...
##        # delpass tinytext,# Cannot retrieve
